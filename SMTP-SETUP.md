...
يجب أن يعيد الخادم كائن JSON يحتوي على `ok: true`. لا يعتبر الفحص ناجحًا إلا إذا كانت Firebase Admin وSMTP وCloudinary مهيأة ومتاحة. جرّب طلب إعادة التعيين من واجهة الموقع بعد التأكد من أن الحساب يستخدم بريدًا إلكترونيًا حقيقيًا. الحسابات القديمة المرتبطة برقم هاتف أو البريد الداخلي لا يمكن إرسال رابط Outlook إليها قبل إضافة بريد استرداد حقيقي.
